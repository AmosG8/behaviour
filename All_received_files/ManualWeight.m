...
if numel(miceWeightDataset(RowInTarget).Weight)>1  
    ...
    DiffFromDayOne=miceWeightDataset(RowInTarget).Weight(end)/miceWeightDataset(RowInTarget).Weight(1);
    if numel(miceWeightDataset(RowInTarget).FoodGiven())>0
      fprintf('last time the mouse weight was %5.2f\n It received %5.2fg of food \n Since then the mouse gained %5.2f. It is now  %5.2f from day1 \n',...
          miceWeightDataset(RowInTarget).Weight(end-1),  miceWeightDataset(RowInTarget).FoodGiven(end), Difference, DiffFromDayOne); 
    end
  else
   Difference=0;
